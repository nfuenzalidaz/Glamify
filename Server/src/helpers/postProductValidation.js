...
const postProductValidation = (name, description, image, price, stock, category, gender ) => {

    // name
    //if (!nameRegex.test(name)) throw new Error(`El nombre ${name} no es válido para crear el producto.`);
    //if (name === "" || name.length === 0) throw new Error(`El campo "nombre" del producto no puede estar vacío.`);
    ...
    // price
    //if (!priceRegex.test(price)) throw new Error('El precio debe ser numerico.');
    //if (price === "") throw new Error('El campo "precio" del producto no puede estar vacío.');
